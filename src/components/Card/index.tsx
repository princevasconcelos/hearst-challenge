...
const CardDetails: React.FC<CardDetailsProps> = ({
  ...
  const uploadForThisData = uploads.find(
    (f) => f.originalImage?.id === data?.image?.id
  );
  ...
  const handleUploadImage = () => {
    ...
    dispatch(
      postImage({
        file: localFile,
        image: hasRequiredProps ? data.image : undefined,
      })
    );
  };
    ...
    >
      <Box sx={{ position: 'relative' }}>
        <LazyLoadImage
          ...
          placeholder={
            <Skeleton variant="rectangular" width={270} height={360} />
          }
          loading="lazy"
      ...
      <CardContent>
        ...
        {expanded && (
          ...
          >
            ...
            />
            <Button
              disabled={uploadStatus === 'loading' || !localFile}
              variant="contained"
            ...
            <Button
              disabled={uploadStatus === 'loading' || !uploadForThisData}
              variant="outlined"
        ...
        {uploadError && expanded && (
          <Alert
            sx={{ maxWidth: '350px', margin: '16px auto 0' }}
            severity="error"
          >
            {uploadError}
          </Alert>
            ...
            ratings.map((rating) => (
              <Typography paragraph key={rating.key}>
                ...
                <Rating
                  ...
                  emptyIcon={<DoNotDisturbIcon fontSize="inherit" />}
                  name={rating.key}
                  value={rating.value}
                  readOnly
                />
