...
describe('cat reducer', () => {
  ...
  it('should handle update breeds action', () => {
    expect(reducer(initialState, updateBreeds(MOCK_BREEDS))).toEqual({
      ...initialState,
      breeds: MOCK_BREEDS,
    });
  });
  describe('createAsyncThunk', () => {
    ...
    it('deleteFavorite', async () => {
      ...
      const apiSpy = jest
        .spyOn(thecatapi, 'deleteFavorite')
        // @ts-ignore
        .mockResolvedValue();
  ...
  describe('Favourites', () => {
    it('should set favourites data when getFavourites is fulfilled', () => {
      const action = {
        type: getFavourites.fulfilled.type,
        payload: MOCK_FAVOURITE_LIST,
      };
      const state = reducer(initialState, action);
      expect(state).toEqual({
        ...initialState,
    ...
    it('should push new favourite when postFavorite is fulfilled', () => {
      ...
      expect(state).toEqual({
        ...initialState,
        favourites: [MOCK_FAVOURITE_RESPONSE],
      });
    });
    ...
    it('should remove favourite when deleteFavorite is fulfilled', () => {
      ...
      const state = reducer(
        { ...initialState, favourites: MOCK_FAVOURITE_LIST },
        action
      );
